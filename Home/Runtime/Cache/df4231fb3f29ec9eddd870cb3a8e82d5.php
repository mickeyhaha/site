		...
		for(var i = 1; i < 3; i++){
			if (i == sign){
	            ...
	            <div class="col-md-8" id="c2">
	            	<!-- 企业动态详细信息 -->
	              	<?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><p class="news-title"><?php echo ($vo["title"]); ?></p>
	              	    <p class="news-status"><?php echo ($vo["create_time"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;来源: <?php echo ($vo["source"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;阅读: <?php echo ($vo["read_times"]); ?>次</p>
	              	    <?php echo (htmlspecialchars_decode($vo["content"])); ?>  
	              	    <p class="news-author">[责任编辑: <?php echo ($vo["author"]); ?>]</p><?php endforeach; endif; else: echo "" ;endif; ?>

					<!-- 先判断是否存在上一篇或下一篇 -->
					<?php if(($prev != 0)): ?><p>上一篇: <a href="__APP__/News/enterprise_info/id/<?php echo ($prev); ?>"><?php echo ($prev_title); ?></a></p><?php endif; ?>
					<?php if(($next != 0)): ?><p>下一篇: <a href="__APP__/News/enterprise_info/id/<?php echo ($next); ?>"><?php echo ($next_title); ?></a></p><?php endif; ?> 	
	            </div>
	        </div>
	    </div>
	</div>
